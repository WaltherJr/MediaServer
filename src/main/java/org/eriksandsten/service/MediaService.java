...
import org.eriksandsten.devtools.ChromeClient;
...
public class MediaService {
    ...
    private final Win32Helper win32Helper = new Win32Helper();
    ...
    private void testWin32Helper(String className) {
        List<Long> processIds = win32Helper.getProcessIdsByName(className);
        // var a = win32Helper.helloWorld(processIds.get(0).intValue());
        List<Long> distinctProcessIds = processIds.stream().distinct().toList();
        var windowHandlesByClassName = win32Helper.getWindowHandlesByWindowClassName(className);
        var windowCaptionsByClassName = win32Helper.getWindowCaptionsByWindowClassName(className);
